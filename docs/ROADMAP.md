...
DECISION: **a provider that publishes its own catalogue outranks the matrix.** Added 2026-08-25, when OpenRouter went in properly. The matrix holds models somebody checked by hand; OpenRouter routes to four hundred and nineteen, of which the matrix knows a handful, so every one of the rest fell to `FALLBACK_CAPABILITIES` and priced as `unknown` — and by the decision above, an unpriced model cannot be held to a spend cap. "CHIMERA supports OpenRouter" therefore quietly meant "budgets do not apply to any of it". `ModelDescriptor` now carries an optional `capabilities` block, `importCatalogue` stores it per connection, and `buildCapabilitiesLookup` merges it over the static record field by field. Reading OpenRouter's published numbers gave 414 of 419 models a verified price. Three constraints hold this in place: the merge is field-by-field, so a provider that publishes price but is silent on vision cannot erase what the matrix knows; `undefined` means "did not say" and never overwrites, while `null` on a numeric field is a real answer and does; and `verifiedAt` records the day the number was read off the provider, not the day the code was written. The parsing lives in the OpenRouter adapter, per the hard rule that provider differences live in adapters only — nothing above `chimera-providers` learns that OpenRouter exists.
...
- **"Swarm" now means a simulated population, MiroFish-style; the orchestrator-and-workers node is a "team".** They are two different animals and the word belongs to the one everybody now means by it. Saved automations keep working through a read-time alias in `normaliseType` — a rename that breaks every file somebody already saved is a deletion, and rewriting every stored `definition_json` is a migration that can corrupt data over a rename that changes no behaviour.
- **A swarm run picks one of two fidelity modes by a threshold the user sets, and always says which it used.** `everyone` calls a model for every agent every round — honest, small, right for tens. `archetypes` has two dozen think and propagates their positions across the population by influence weight — right for thousands, where two thousand agents cost seventy-two calls. Nobody runs a million agents through a language model; that arithmetic is a five-figure bill per question, and a swarm that would not say how its percentage was produced is one nobody should act on.
- **Every Composio tool is irreversible until proven otherwise, and nothing proves otherwise.** `composio.execute` reaches several thousand actions across several hundred apps and is how an agent sends the Gmail or charges the card. A slug heuristic (`FETCH`/`LIST`/`GET` are reads) was considered and rejected: right most of the time, and the cost of the exception is a message nobody can recall. It sits in `ALWAYS` beside `shell.exec` and `email.send`.
- **The home screen's assistant is an ordinary agent with a read-only view of the workspace, not a privileged path.** It could design an automation and knew nothing else — ask it what a run cost or which agents can send email and it had no way to find out, though nothing was missing from the database. It now runs on `runAgentLoop` with the `assistant` role and a `workspace` MCP server covering automations, agents, runs, notes, plugins, providers, templates and granted folders. Read-only by construction: no tool on that server writes, and `planAutomation` returns a design and applies nothing. This also closed a standing violation of CLAUDE.md's first hard rule — the old chat called `adapter.streamChat` directly, with no Governor, as does nothing now.
- **Document parsers run in a child process, never in the app.** `exceljs`, `mammoth`, `pdfjs-dist` and `yauzl` read files that arrived from a stranger, which is the shape of most document-parser CVEs. The child gets a path, a limit, a timeout and an environment holding only `PATH`. Spreadsheets are rendered as TSV per sheet rather than flattened, because a data extractor asked which column holds the totals cannot answer from cells in reading order. Not SheetJS despite it being the better library: the npm `xlsx` package is the abandoned build carrying two unfixed high-severity advisories.
- **Web search (`search.web`) is its own tool server, keyless by default, with an optional keyed API — added outside the milestone plan because the product did not work without it.** The plan gave the researcher `http.request` and no way to find a URL, so every research automation needed a person to do the finding first. Keyless stays the default so a fresh workspace works with no account; it is scraped, and scraped search is at the mercy of somebody else's bot policy — measured from a datacentre address, Bing answered a five-word query as though it were the first word, Mojeek captcha'd after a handful of requests, and every public SearXNG instance returned 429. A workspace that needs research to work every time names Brave, Tavily or Serper in Providers. Deliberately not a mode of `http.request`: finding a page and sending to one are different permissions, and search has no SSRF surface because the model never names the host.
- **A satisfied JSON output contract is the verification for that step; only steps producing prose are verified by a model.** A schema is a check, and a model's opinion of whether the work is done is not. Measured live: the planner produced a valid plan on its first turn three runs running and was told "the steps were only listed and no actual data was retrieved" — graded on whether the plan had been *carried out*, which its own system prompt says it does not do. It exhausted every time, having been right at iteration zero every time. This is stricter than the model's answer, saves a call per contract-bearing step, and cannot drift onto the wrong question.
